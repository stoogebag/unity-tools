    ...
    void DrawNodeWindow(int id)
    {
        ...
        for (int i = 0; i < db.playerDiags[id].comment.Count; i++)
        {
            if (db.playerDiags[id].comment.Count > 0)
            {
                ...
                var recordLabel = db.playerDiags[id].comment[i].recording ? "S" : "R";
                var transcribe = false;
                if (GUILayout.Button(recordLabel, GUILayout.Width(20)))
                {
                    //Undo.RecordObject(db, "Show Audio and Sprites");

                    var recording = !db.playerDiags[id].comment[i].recording; 
                    
                    db.playerDiags[id].comment[i].recording = recording;
                ...
                {
                    Transcribe(db.playerDiags[id].comment[i].audios, id, i);
                }
