...
public class VIDEUIManagerStooge : MonoBehaviour
{
    ...
    public static bool HasCoffee;
    public static bool HasPhoneCharged;
            ...
            if (!data.isPlayer)
            {
                ...
                if (data.extraVars.ContainsKey("item") && !data.dirty)
                {
                    if (data.commentIndex == (int)data.extraVars["itemLine"])
                    {
                        if (data.extraVars
                            .ContainsKey(
                                "item++")) //If we have this key, we use it to increment the value of 'item' by 'item++'
                        {
