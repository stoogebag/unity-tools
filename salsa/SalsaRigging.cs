...
namespace stoogebag._2dConvos
{
    public class SalsaRigging : MonoBehaviour
    {
        ...
        [SerializeField] private bool hideUnusedSprites = true;


        [SerializeField] private SpriteRenderer[] invisibleOnBlink;
        ...
        public void Rig(AudioSource source = null, float eyeRangeOfMotion = .01f)
        {
            ...
            if (TrimWhitespace) target.StripWhitespaceFromChildNames();
            ...
            var movingEyes = allChildren.Where(t =>
                t.name == movingEyeString + "R" || t.name == movingEyeString + "L" || t.name == movingEyeString);
            var disappearOnBlink =
                allChildren.Where(t =>
                    (t.name.Contains(eyeNoBlinkString) || t.name.Contains(movingEyeString)) &&
                    !t.name.Contains(eyeLidString));
...
#if UNITY_EDITOR
            EditorUtility.SetDirty(randomEyes);
