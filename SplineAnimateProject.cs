...
using UnityEditor;
    ...
    /// </summary>
    [AddComponentMenu("Splines/Spline Animate Projecct")]
    [ExecuteInEditMode]
    public class SplineAnimateProject : SplineComponent
    {
        /// <summary>
        ...
        void EvaluatePositionAndRotation(out Vector3 position, out Quaternion rotation)
        {
            ...
            position = GetTopMostPosition(position);
        ...
        private Vector3 GetTopMostPosition(Vector3 position)
        {
            
            //raycost nonalloc downward to find the topmost position on theCollider
            //RaycastHit hit;
            //only cast hitting theCollider
            Ray ray = new Ray(position+Vector3.up*10, Vector3.down);
            if (theCollider.Raycast(ray, out var hit, 50))
            {
                return hit.point;
            }
            return position;
